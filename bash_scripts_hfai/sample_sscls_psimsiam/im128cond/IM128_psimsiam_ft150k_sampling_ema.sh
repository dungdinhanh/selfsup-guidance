...
--model_path models/128x128_diffusion.pt $SAMPLE_FLAGS  --logdir runs/ft_negweight/sampling_ema/IMN128_sft${ft}/conditional/scale${scale}/"
echo ${cmd}
eval ${cmd}
done
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet128_labeled.npz \
 runs/ft_negweight/sampling_ema/IMN128_sft${ft}/conditional/scale${scale}/reference/samples_50000x128x128x3.npz"
echo ${cmd}
eval ${cmd}
done
done
