...
--classifier_path ss_models/p100k_ss_cls.pt --model_path models/64x64_diffusion_unc.pt $SAMPLE_FLAGS \
 --logdir runs/sampling_psimsiam/IMN64/unconditional/scale${scale}/ "
echo ${cmd}
eval ${cmd}
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet64_labeled.npz \
 runs/sampling_psimsiam/IMN64/unconditional/scale${scale}/reference/samples_50000x64x64x3.npz"
echo ${cmd}
eval ${cmd}
done
