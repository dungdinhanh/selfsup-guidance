...
SAMPLE_FLAGS="--batch_size 50 --num_samples 30000 --timestep_respacing 250"
...
do
  ...
  do
    ...
    do
cmd="python scripts_glide/robust/glide_up_sample_prog_extcap.py $MODEL_FLAGS --guidance_scale ${scale} \
 --ext_captions eval_models/pretext2img/reference/captions_${extcapt}_512.npz --eps ${eps}  $SAMPLE_FLAGS \
 --logdir runs/sampling_glide_retest/IMN256/scale${scale}_eps${eps}_ec${extcapt}/ "
echo ${cmd}
eval ${cmd}
done
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_MSCOCO_val_64x64_squ.npz \
 runs/sampling_glide_retest/IMN256/scale${scale}_eps${eps}_ec${extcapt}/reference/samples_30000x64x64x3.npz"
echo ${cmd}
eval ${cmd}
done
done
