...
scales=( "10.0" "12.0" "14.0" "16.0" "18.0")
#scales=( "1.0"  )
jointtemps=( "0.3")
margintemps=( "0.3" )
...
do
cmd="python script_odiff/mocov2_meanclose_sup_sample_transform_shortguidance_cdiv.py $MODEL_FLAGS --classifier_scale ${scale}  \
--classifier_type mocov2 --model_path models/64x64_diffusion_unc.pt $SAMPLE_FLAGS --joint_temperature ${jt} \
 --logdir runs/sampling_ots/IMN64/unconditional/scale${scale}_jointtemp${jt}_margtemp${mt}_mocov2_meanclose_shg_cdiv/ \
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet64_labeled.npz \
 runs/sampling_ots/IMN64/unconditional/scale${scale}_jointtemp${jt}_margtemp${mt}_mocov2_meanclose_shg_cdiv/reference/samples_50000x64x64x3.npz"
echo ${cmd}
eval ${cmd}
done
done
