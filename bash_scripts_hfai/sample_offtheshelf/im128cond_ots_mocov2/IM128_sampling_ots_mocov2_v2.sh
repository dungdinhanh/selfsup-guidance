...
scales=( "4.5" "5.0" "5.5" "6.0" "6.5" "7.0" )
jointtemps=( "0.3")
margintemps=( "0.3" )
...
do
cmd="python script_odiff/mocov2_mean_sup_sample_transform.py $MODEL_FLAGS --classifier_scale ${scale} \
...
--logdir runs/sampling_ots/IMN128/conditional/scale${scale}_jointtemp${jt}_margtemp${mt}_mocov2/ \
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet128_labeled.npz \
 runs/sampling_ots/IMN128/conditional/scale${scale}_jointtemp${jt}_margtemp${mt}_mocov2/reference/samples_50000x128x128x3.npz"
echo ${cmd}
eval ${cmd}
done
done
