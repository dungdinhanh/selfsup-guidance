...
--model_path models/128x128_diffusion.pt $SAMPLE_FLAGS --joint_temperature ${jt} --margin_temperature_discount ${mt}  --softplus_beta ${sb} \
--logdir runs/sampling_ots/IMN128/conditional/scale${scale}_jointtemp${jt}_margtemp${mt}_softbeta${sb}/ "
echo ${cmd}
eval ${cmd}
done
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet128_labeled.npz \
 runs/sampling_ots/IMN128/conditional/scale${scale}_jointtemp${jt}_margtemp${mt}_softbeta${sb}/reference/samples_50000x128x128x3.npz"
echo ${cmd}
eval ${cmd}
done
done
