...
cmd="python script_odiff/mocov2_meanclose_contrastive_outclass_sup_instance_sample_transform.py $MODEL_FLAGS --classifier_scale ${scale}  \
 ...
 --logdir runs/sampling_ots_contrastive_outclass2/IMN256/kc${kc}/conditional/scale${scale}_jt${jt}_mocov2_mean_close/ --features eval_models/imn256_mocov2/reps3.npz --k_closest 10"
echo ${cmd}
eval ${cmd}
done
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet256_labeled.npz \
 runs/sampling_ots_contrastive_outclass2/IMN256/kc${kc}/conditional/scale${scale}_jt${jt}_mocov2_mean_close/reference/samples_50000x256x256x3.npz"
echo ${cmd}
eval ${cmd}
done
done
