...
cmd="python script_odiff/classifier_sample.py $MODEL_FLAGS --classifier_scale ${scale}  \
 --classifier_type resnet50 --model_path models/256x256_diffusion.pt $SAMPLE_FLAGS --joint_temperature ${jt}\
 --logdir runs/sampling_ots/IMN256/conditional/scale${scale}_jointtemps${jt}/ "
echo ${cmd}
eval ${cmd}
done
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet256_labeled.npz \
 runs/sampling_ots/IMN256/conditional/scale${scale}_jointtemps${jt}/reference/samples_50000x256x256x3.npz"
echo ${cmd}
eval ${cmd}
done
done
