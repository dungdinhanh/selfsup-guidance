...
cmd="python script_odiff/mocov2_meanclose_sup_sample_transform_cdiv.py $MODEL_FLAGS --classifier_scale ${scale}  \
 ...
 --logdir runs/sampling_ots/IMN256/unconditional/scale${scale}_mocov2_mean_close_cdiv/ --features eval_models/imn256_mocov2/reps3.npz"
echo ${cmd}
eval ${cmd}
done
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet256_labeled.npz \
 runs/sampling_ots/IMN256/unconditional/scale${scale}_mocov2_mean_close_cdiv/reference/samples_50000x256x256x3.npz"
echo ${cmd}
eval ${cmd}
done
done
