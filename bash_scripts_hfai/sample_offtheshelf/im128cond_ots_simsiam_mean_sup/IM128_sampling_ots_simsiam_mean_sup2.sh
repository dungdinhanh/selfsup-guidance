...
do
cmd="python script_odiff/simsiam_mean_sup_sample_transform.py $MODEL_FLAGS --classifier_scale ${scale} \
...
--logdir runs/sampling_ots/IMN128/conditional/scale${scale}_jointtemp${jt}_margtemp${mt}_simsiam_mean_sup/ \
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet128_labeled.npz \
 runs/sampling_ots/IMN128/conditional/scale${scale}_jointtemp${jt}_margtemp${mt}_simsiam_mean_sup/reference/samples_50000x128x128x3.npz"
echo ${cmd}
eval ${cmd}
done
done
