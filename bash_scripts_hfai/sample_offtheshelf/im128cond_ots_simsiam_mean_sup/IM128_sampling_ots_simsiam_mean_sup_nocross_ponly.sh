...
do
cmd="python script_odiff/simsiam_mean_sup_sample_transform_nocross_onlyp.py $MODEL_FLAGS --classifier_scale ${scale} \
...
--logdir runs/sampling_ots/IMN128/conditional/scale${scale}_jointtemp${jt}_margtemp${mt}_simsiam_mean_sup_nocross_ponly/ \
--features eval_models/imn128_simsiam/reps3.npz --save_imgs_for_visualization False"
echo ${cmd}
eval ${cmd}
done
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet128_labeled.npz \
 runs/sampling_ots/IMN128/conditional/scale${scale}_jointtemp${jt}_margtemp${mt}_simsiam_mean_sup_nocross_ponly/reference/samples_50000x128x128x3.npz"
echo ${cmd}
eval ${cmd}
done
done
