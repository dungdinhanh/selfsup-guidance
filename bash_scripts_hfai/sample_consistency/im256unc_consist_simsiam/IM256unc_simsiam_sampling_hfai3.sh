...
for scale in "${scales[@]}"
do
cmd="python scripts_gdiff/consistency/classifier_sample_selfrep2_simsiam.py $MODEL_FLAGS $SAMPLE_FLAGS --classifier_scale ${scale}  \
--classifier_path eval_models/simsiam_0099.pth.tar --model_path models/256x256_diffusion_uncond.pt  \
 --logdir runs/sampling_selfrep_simsiam/IMN256/unconditional/scale${scale}/ --features eval_models/imn256_simsiam/reps3.npz"
echo ${cmd}
eval ${cmd}
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet256_labeled.npz \
 runs/sampling_selfrep_simsiam/IMN256/unconditional/scale${scale}/reference/samples_50000x256x256x3.npz"
echo ${cmd}
eval ${cmd}
done
