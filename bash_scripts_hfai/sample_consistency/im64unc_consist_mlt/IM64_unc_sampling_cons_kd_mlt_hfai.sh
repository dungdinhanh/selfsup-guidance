...
#SAMPLE_FLAGS="--batch_size 60 --num_samples 50000 --timestep_respacing 250"
#SAMPLE_FLAGS="--batch_size 52 --num_samples 50000 --timestep_respacing 250"
...
for scale in "${scales[@]}"
do
cmd="python scripts_gdiff/consistency/classifier_sample_selfrep_mlt.py $MODEL_FLAGS --classifier_scale ${scale}  \
--classifier_path models/64x64_classifier.pt --model_path models/64x64_diffusion_unc.pt $SAMPLE_FLAGS \
 --logdir runs/sampling_selfrep_kd_mlt/IMN64/unconditional/scale${scale}/ --classifier_depth 4 --features eval_models/imn64/reps2.npz "
echo ${cmd}
eval ${cmd}
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet64_labeled.npz \
 runs/sampling_selfrep_kd_mlt/IMN64/unconditional/scale${scale}/reference/samples_50000x64x64x3.npz"
echo ${cmd}
eval ${cmd}
done
