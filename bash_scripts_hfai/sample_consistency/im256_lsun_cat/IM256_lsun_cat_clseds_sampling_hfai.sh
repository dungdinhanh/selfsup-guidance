...
--classifier_path models/256x256_classifier.pt $MODEL_FLAGS --model_path models/lsun_cat.pt $SAMPLE_FLAGS\
 --logdir runs/sampling_LSUN_cat/IMN256/conditional/scale${scale}/ "
echo ${cmd}
eval ${cmd}
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_lsun_bedroom256.npz \
 runs/sampling_LSUN_cat/IMN256/conditional/scale${scale}/reference/samples_50000x256x256x3.npz"
echo ${cmd}
eval ${cmd}
done
