...
for scale in "${scales[@]}"
do
cmd="python scripts_gdiff/conflict2/classifier_sample_consistency4.py $MODEL_FLAGS --classifier_scale ${scale}  \
--classifier_path models/64x64_classifier.pt --model_path models/64x64_diffusion_unc.pt $SAMPLE_FLAGS \
 --logdir runs/sampling_consistency_blocklg2/IMN64/unconditional/scale${scale}/ --classifier_depth 4"
echo ${cmd}
eval ${cmd}
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet64_labeled.npz \
 runs/sampling_consistency_blocklg2/IMN64/unconditional/scale${scale}/reference/samples_50000x64x64x3.npz"
echo ${cmd}
eval ${cmd}
done
