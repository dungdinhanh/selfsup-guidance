...
for scale in "${scales[@]}"
do
cmd="python scripts_gdiff/conflict2/classifier_sample_consistency.py $MODEL_FLAGS --classifier_scale ${scale}  \
--classifier_path models/256x256_classifier.pt --model_path models/256x256_diffusion.pt $SAMPLE_FLAGS \
 --logdir runs/sampling_consistency/IMN256/conditional/scale${scale}/ "
echo ${cmd}
eval ${cmd}
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet256_labeled.npz \
 runs/sampling_consistency/IMN256/conditional/scale${scale}/reference/samples_50000x256x256x3.npz"
echo ${cmd}
eval ${cmd}
done
