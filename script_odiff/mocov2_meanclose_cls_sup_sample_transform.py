...
def process_features(features_p, labels):
    n = features_p.shape[0]
    new_features_p = []
    new_labels = []
    ...
    for i in range(n):
        if labels[i] == current_label:
            continue
        features_p_current_class = features_p[current_index:i]
        new_features_p.append(features_p_current_class)
        new_labels.append(current_label)
        current_label = labels[i]
        current_index = i
    return np.stack(new_features_p), np.stack(labels)
