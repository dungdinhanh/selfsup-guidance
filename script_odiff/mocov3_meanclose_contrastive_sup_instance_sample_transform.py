...
from scripts_gdiff.selfsup.support.script_util_ss import create_simsiam_selfsup, simsiam_defaults, create_mocov2_selfsup, create_mocov3_selfsup
from scripts_gdiff.selfsup.support.dist_util import load_simsiam, load_mocov2, load_mocov3
...
def main(local_rank):
    ...
    assert args.classifier_type in ['finetune', 'mocov3', 'resnet50', 'resnet101', 'simsiam', 'mocov2']
    ...
    if args.classifier_type == 'finetune':
        ...
        resnet.eval()
        resnet.sampling=True
    elif args.classifier_type=='mocov3':
        resnet_address = 'eval_models/mocov3_r_50_1000ep.pth.tar'
        resnet = create_mocov3_selfsup(**args_to_dict(args, simsiam_defaults().keys()))
        for param in resnet.parameters():
            param.required_grad = False
        resnet.load_state_dict(load_mocov3(resnet_address))
        resnet.to(dist_util.dev())
        resnet.eval()
        resnet.sampling=True
    else:
