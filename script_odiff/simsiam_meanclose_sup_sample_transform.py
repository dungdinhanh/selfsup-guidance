...
def main(local_rank):
    ...
    if not os.path.isfile(features_mean_sup_file):
        ...
        features_p, features_z, labels_associated = get_mean_closest_sup(features_p, features_z, labels_associated, k=args.k_closest)
...
def get_mean_closest_sup(features_p, features_z, labels, k=5):
    ...
    for key in dict_p_classes.keys():
        ...
        z_closest_vectors = z_vectors[indexes]
        p_mean_vectors.append(p_closest_vectors)
        z_mean_vectors.append(z_closest_vectors)
    # print(p_mean_vectors[0].shape)
    # print(p_mean_vectors[50].shape)
    # print(labels_vectors[0])
    # print(labels_vectors[50])
    return np.concatenate(p_mean_vectors), np.concatenate(z_mean_vectors), np.concatenate(labels_vectors)
