...
def main(local_rank):
    ...
    def design_cond_fn(inputs, t, y=None, p_features=None, selected_indexes=None):
        ...
        with th.enable_grad():
            ...
            numerator_t = th.exp(logits * temperature1)
            denominator_t = th.exp(logits * temperature2).sum(1, keepdims=True)
            t_selected = th.log(numerator_t / denominator_t)

            numerator = th.exp(logits * temperature1)
            denominator2 = th.exp(logits * temperature2).sum(1, keepdims=True)

            selected = th.log(numerator / denominator2)

            loss = (selected * t_selected).sum(1, keepdims=True)
            ...
            # denominator2 = th.exp(logits*temperature2).sum(1, keepdims=True)
            # selected = th.log(numerator / denominator2)
            return th.autograd.grad(loss.sum(), pred_xstart_r)[0] * args.classifier_scale
