...
def main(local_rank):
    ...
    features_mean_sup_file = os.path.join(features_folder, "reps3_mean_sup.npz")
    if not os.path.isfile(features_mean_sup_file):
        ...
        features_p, features_z, labels_associated = get_mean_sup(features_p, features_z, labels_associated)
        np.savez(features_mean_sup_file, features_p, features_z, labels_associated)
    ...
    def design_cond_fn(inputs, t, y=None, p_features=None, z_features=None):
        ...
        with th.enable_grad():
            ...
            match1 = similarity_match(p_features.detach(), z_x_0)
            match2 = similarity_match(p_x_0, z_features.detach())
...
def get_mean_sup(features_p, features_z, labels):
    ...
    for i in range(n):
        ...
        if class_label not in dict_p_classes:
            ...
            dict_z_classes[class_label] = []
        dict_p_classes[class_label].append(features_p[i])
        dict_z_classes[class_label].append(features_z[i])
    for key in dict_p_classes.keys():
        p_vectors = np.stack(dict_p_classes[key])
        z_vectors = np.stack(dict_z_classes[key])
        p_mean = np.mean(p_vectors, axis=0)
        z_mean = np.mean(z_vectors, axis=0)
        p_mean_vectors.append(p_mean)
        z_mean_vectors.append(z_mean)
        labels_vectors.append(key)
    return np.stack(p_mean_vectors), np.stack(z_mean_vectors), np.stack(labels)
