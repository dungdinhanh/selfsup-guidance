...
def color_jitter(image, brightness=0.4, contrast=0.4, saturation=0.4, hue=0.4, p=0.5):
    # Convert PIL image to PyTorch tensor
    if th.rand(1).item() < p:
        # Apply brightness
        image = F.adjust_brightness(image, 1 + brightness * (th.rand(1).item() * 2 - 1))

        # Apply contrast
        image = F.adjust_contrast(image, 1 + contrast * (th.rand(1).item() * 2 - 1))

        # Apply saturation
        image = F.adjust_saturation(image, 1 + saturation * (th.rand(1).item() * 2 - 1))

        # Apply hue
        image = F.adjust_hue(image, hue * (th.rand(1).item() * 2 - 1))
...
def random_grayscale(image, p=0.5):
    # Apply random grayscale with probability p
    # if th.rand(1).item() < p:
    #     # Convert to grayscale
    #     image = F.rgb_to_grayscale(image)
    #
    #     # Repeat the single-channel grayscale image to match the original number of channels
    #     image = image.repeat(3, 1, 1)
    return image

def random_horizontal_flip(image, probability=0.5):
    if th.rand(1).item() < probability:
        return th.flip(image, [2])
    else:
        return image
...
def main(local_rank):
    ...
    def design_cond_fn(inputs, t, y=None, p_features=None, z_features=None):
        ...
        with th.enable_grad():
            ...
            pred_xstart_raw = ((pred_xstart + 1) * 127.).clamp(0, 255)/255.0
            pred_xstart_r = center_crop_arr(pred_xstart_raw, min(args.image_size, 224))
            pred_xstart_r = custom_normalize(pred_xstart_r, mean_imn, std_imn)

            pred_xstart_aug = color_jitter(pred_xstart_raw)
            pred_xstart_aug = random_grayscale(pred_xstart_aug, p=0.2)
            pred_xstart_aug = random_horizontal_flip(pred_xstart_aug)
            pred_xstart_aug = custom_normalize(pred_xstart_aug, mean_imn, std_imn)


            # resnet classifier
            p_x_0,  z_x_0= resnet(pred_xstart_r)
            p_x_1, z_x_1 = resnet(pred_xstart_aug)
            match1 = similarity_match(p_x_1, z_x_0.detach())
            match2 = similarity_match(p_x_0, z_x_1.detach())
            match = (match1 + match2) * 0.5
            # # temperature
            ...
            gradients = th.autograd.grad(match.sum(), [pred_xstart_r, pred_xstart_aug])

            return (0.5 * gradients[0] + 0.5 * gradients[1]) * args.classifier_scale
