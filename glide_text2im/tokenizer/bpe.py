...
class Encoder:
    ...
    def encode_batch(self, text_list):
        ...
        for i in range(len(text_list)):
            list_tokens.append(self.encode(text_list[i]))
        return list_tokens

    def encode_batch_others(self, other_texts_list):
        list_other_tokens = []
        for i in range(len(other_texts_list)):
            other_tokens = []
            for j in range(len(other_texts_list[i])):
                other_tokens.append(other_texts_list[i][j])
            list_other_tokens.append(other_tokens)
        return list_other_tokens

    def padded_tokens_and_mask_batch(
        self, batch_tokens: List[List], text_ctx: int
    ) -> Tuple[List[List[int]], List[List[bool]]]:
        batch_padded_tokens = []
        batch_padded_masks = []
        n = len(batch_tokens)
        for i in range(n):
            padded_tokens, padded_masks = self.padded_tokens_and_mask(batch_tokens[i], text_ctx)
            batch_padded_tokens.append(padded_tokens)
            batch_padded_masks.append(padded_masks)
        return batch_padded_tokens, batch_padded_masks
