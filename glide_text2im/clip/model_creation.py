...
class CLIPModelRep:
    ...
    def cond_fn(self, prompts: List[str], grad_scale: float, prompts_others: List[List[str]]) -> Callable[..., torch.Tensor]:
        with torch.no_grad():
            z_t = self.text_embeddings(prompts)
            z_t_others = self.text_embeddings_others(prompts_others)
            z_t_1 = torch.unsqueeze(z_t, dim=1)
            all_z_t = torch.concat((z_t_1, z_t_others), dim=1)
            n_z = all_z_t.shape[1]
            batch_size = all_z_t.shape[0]

        def cond_fn(x, t, grad_scale=grad_scale, osc=0.9, **kwargs):
            indexes  = np.arange(n_z)
            main_prob  = osc
            other_prob = (1-osc)/(n_z - 1)
            probs = np.ones(n_z, dtype=float)
            probs *= other_prob
            probs[0] = main_prob
            random_indexes = torch.from_numpy(np.random.choice(indexes, batch_size, p=probs)).to(self.device)
            associated_indexes = torch.arange(batch_size).to(self.device)
            # a_random_indexes = torch.vstack((associated_indexes, random_indexes)).transpose(1,0)


            with torch.enable_grad():
                x_var = x.detach().requires_grad_(True)
                z_i = self.image_embeddings(x_var, t)
                z_t_centroid = all_z_t[associated_indexes, random_indexes]
                # print(a_random_indexes.shape)
                # print(z_t_centroid.shape)
                # z_t_centroid = z_t * osc + z_t_others * (1-osc)/4
                loss = torch.exp(self.logit_scale) * (z_t_centroid * z_i).sum()
                grad = torch.autograd.grad(loss, x_var)[0].detach()
            return grad * grad_scale
