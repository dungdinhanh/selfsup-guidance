...
class GaussianDiffusionSHG_LG(SpacedDiffusion):
    ...
    def __init__(self,
        ...
        *,
        ...
        super(GaussianDiffusionSHG_LG, self).__init__(use_timesteps=use_timesteps, betas=betas,
                                                   model_mean_type=model_mean_type,
                                                   model_var_type=model_var_type,
                                                   loss_type=loss_type,
                                                   rescale_timesteps=rescale_timesteps)
    ...
    def p_sample(
        ...
        if cond_fn is not None:
            ...
            gradient_x0, gradient_xt = cond_fn([x,pred_xstart], self._scale_timesteps(t), **model_kwargs)
            out["mean"] = (
                out["mean"].float() + (out["variance"]+ add_value)* gradient_xt.float() + (out["variance"]+add_value)*gradient_x0.float()
            )
...
class GaussianDiffusionSHG_LGCDiv(GaussianDiffusionSHG_LG):
    ...
    def __init__(self,
        ...
        *,
        ...
        super(GaussianDiffusionSHG_LGCDiv, self).__init__(use_timesteps=use_timesteps, betas=betas,
                                                   model_mean_type=model_mean_type,
                                                   model_var_type=model_var_type,
                                                   loss_type=loss_type,
                                                   rescale_timesteps=rescale_timesteps)
    ...
    def p_sample(
        ...
        if cond_fn is not None:
            ...
            out["mean"], gradient_div = self.condition_mean(
                cond_fn, out, x, t, nonzero_mask, gamma_factor, model_kwargs=model_kwargs
            )
        else:
            noise = th.randn_like(x)
            gradient_div = nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise
    ...
    def condition_mean_mtl(self, cond_fn, p_mean_var, x, t, nonzero_mask, gamma_factor, model_kwargs=None):
        """
        Compute the mean for the previous step, given a function cond_fn that
        computes the gradient of a conditional log probability with respect to
        x. In particular, cond_fn computes grad(log(p(y|x))), and we want to
        ...
        This uses the conditioning strategy from Sohl-Dickstein et al. (2015).
        """
        gradient_lg, gradient_shg = cond_fn([x, p_mean_var['pred_xstart']], self._scale_timesteps(t), **model_kwargs)
        ...
        gradient_cls_shg = (p_mean_var["variance"] + add_value) * gradient_shg.float()
        gradient_cls_lg = (p_mean_var["variance"] + add_value) * gradient_lg.float()
        gradient_gen = _extract_into_tensor(self.posterior_mean_coef1, t, x.shape) * p_mean_var['pred_xstart']
        ...
        g_cls_shape = gradient_cls_shg.shape
        g_gen_shape = gradient_gen.shape
        assert g_cls_shape == g_gen_shape

        #diffusion - classification conflict
        new_gradient_cls_shg = self.project_conflict(gradient_cls_shg.clone(), gradient_gen.clone(), g_cls_shape) # check if gradient_cls change / changed -> using clone -> does not change
        new_gradient_gen = self.project_conflict(gradient_gen.clone(), gradient_cls_shg.clone(), g_gen_shape) # check if gradient_gen change / changed -> using clone -> does not change

        # classification - diversity conflict
        final_gradient_cls_shg = self.project_conflict(new_gradient_cls_shg.clone(), gradient_div.clone(), g_cls_shape)
        final_gradient_div = self.project_conflict(gradient_div.clone(), new_gradient_cls_shg.clone(), g_cls_shape)

        new_mean = (
            p_mean_var["mean"].float() - gradient_gen.float() + new_gradient_gen.float() + final_gradient_cls_shg.float() + gradient_cls_lg.float()
        )
        del gradient_cls_shg, gradient_cls_lg, gradient_gen, new_gradient_gen, new_gradient_cls_shg, final_gradient_cls_shg
        return new_mean, final_gradient_div
