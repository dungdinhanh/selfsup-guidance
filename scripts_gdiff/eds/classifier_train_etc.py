...
def main(local_rank):
    ...
    def forward_backward_log(data_loader, prefix="train"):
        ...
        for i, (sub_batch, sub_labels, sub_t) in enumerate(
            ...
            losses = {}
            # ce loss
            losses[f"{prefix}_ce_loss_for_classifier1"] = ce_loss.detach()
            ...
            entropy = np.log(NUM_CLASSES) + (log_probs * probs).sum(dim=-1)  # (B,)
            entropy_constraint_loss = args.entropy_constraint_loss_weight * entropy
            losses[f"{prefix}_entropy_constraint_loss_for_classifier1"] = entropy_constraint_loss.detach()
