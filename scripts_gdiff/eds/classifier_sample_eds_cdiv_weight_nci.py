...
# import hfai
from guided_diffusion import dist_util_nci, logger
from eds_guided_diffusion.script_util_mlt import (
    NUM_CLASSES,
    model_and_diffusion_defaults,
    classifier_defaults,
    create_model_and_diffusion_cdiv_weight,
    create_classifier,
...
def main(local_rank):
    ...
    dist_util_nci.setup_dist(local_rank)
    ...
    model, diffusion = create_model_and_diffusion_cdiv_weight(
        **args_to_dict(args, model_and_diffusion_defaults().keys())
    )
    model.load_state_dict(
        dist_util_nci.load_state_dict(args.model_path, map_location="cpu")
    )
    model.to(dist_util_nci.dev())
    if args.use_fp16:
        model.convert_to_fp16()
    model.eval()
    diffusion.w1 = args.w1
    diffusion.w2 = args.w2
    diffusion.w3 = args.w3
    diffusion.w4 = args.w4


    logger.log("loading classifier...")
    classifier = create_classifier(**args_to_dict(args, classifier_defaults().keys()))
    classifier.load_state_dict(
        dist_util_nci.load_state_dict(args.classifier_path, map_location="cpu")
    )
    classifier.to(dist_util_nci.dev())
    ...
    while len(all_images) * args.batch_size < args.num_samples:
        ...
        if args.specified_class is not None:
            classes = th.randint(
                ...
                device=dist_util_nci.dev()
            )
        else:
            classes = th.randint(
                low=0, high=args.num_classes, size=(args.batch_size,), device=dist_util_nci.dev()
            )
