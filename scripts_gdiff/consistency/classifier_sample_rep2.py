...
def main(local_rank):
    ...
    def cond_fn(x, t, y=None):
        ...
        with th.enable_grad():
            ...
            rep = features[1].feature_list.detach() # check if match
            # rep = th.flatten(rep, start_dim=1)
            x_in_transform = transform_act(x_in)
            _ = classifier(x_in_transform, t)
            rep_transform = features[1].feature_list
...
def similarity_match(pred, target, mean=True):
    pred_norm = th.nn.functional.normalize(pred, dim=2)
    target_norm = th.nn.functional.normalize(target, dim=2)
    cosine_sim = (pred_norm * target_norm).sum(dim=2).mean(dim=1)
    loss = cosine_sim
    if mean:
        loss = loss.sum()
    return loss
