...
from guided_diffusion.script_util_con import (
    NUM_CLASSES,
    model_and_diffusion_defaults,
    classifier_defaults,
    create_model_and_diffusion_con,
    create_classifier,
...
def main(local_rank):
    ...
    model, diffusion = create_model_and_diffusion_con(
    ...
    def cond_fn(x, t, y=None, x_den=None, x_div=None, x_start=None, tor=None):
        ...
        with th.enable_grad():
            ...
            x_mean, _, _ = diffusion.q_posterior_mean_variance(x_start, x_in, tor)

            x_den_in = x_mean + x_den
            x_den_div_in = x_mean + x_den + x_div
            logits1 = classifier(x_den_in, t)
            logits2 = classifier(x_den_div_in, t)
            loss = similarity_loss(logits1, logits2.detach())
            # logits = classifier(x_in, t)
            # log_probs = F.log_softmax(logits, dim=-1)
            # selected = log_probs[range(len(logits)), y.view(-1)]
            return -th.autograd.grad(loss, x_in)[0] * args.classifier_scale

    def model_fn(x, t, y=None, x_den=None, x_div=None, x_start=None, tor=None):
        assert y is not None
    ...
    while len(all_images) * args.batch_size < args.num_samples:
        ...
        model_kwargs['x_den'] = None
        model_kwargs['x_div'] = None
        model_kwargs['x_start'] = None
        model_kwargs['tor'] = None
        sample_fn = (
