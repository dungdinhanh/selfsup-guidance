...
from glide_text2im.clip.model_creation_robust import create_clip_robust_model
...
from eds_guided_diffusion.script_util_robust import create_lvae_te, lvae_defaults
...
def main(local_rank):
    ...
    label_vae = create_lvae_te(**args_to_dict(args, lvae_defaults().keys()))
    label_vae.load_state_dict(
        dist_util.load_state_dict(args.lvae_path, map_location="cpu")
    )
    label_vae.to(dist_util.dev())
    label_vae.eval()
    ...
    clip_model = create_clip_robust_model(device=dist_util.dev())
