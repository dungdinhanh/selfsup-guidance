...
from glide_text2im.clip.model_creation import create_clip_model_robust_extcap
...
def main(local_rank):
    ...
    clip_model = create_clip_model_robust_extcap(device=dist_util.dev())
    ...
    def cond_fn(x, t, y=None):
        ...
        with th.enable_grad():
            ...
            z_t = embbed_captions[y]
            z_i = clip_model.image_embeddings(x_in, t)
            loss = th.exp(clip_model.logit_scale) * (z_t * z_i).sum()
            grad = th.autograd.grad(loss, x_in)[0].detach()
