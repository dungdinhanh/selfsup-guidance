...
shg_scales=( "1.0" "3.0" "4.0" "5.0" "6.0" "7.0" )
#scales=( "1.0"  )
jointtemps=( "0.3")
margintemps=( "0.3" )
...
--classifier_type mocov2 --model_path models/64x64_diffusion_unc.pt $SAMPLE_FLAGS --joint_temperature ${jt} \
 --logdir runs/sampling_ots/IMN64/unconditional/scale${scale}_jointtemp${jt}_margtemp${mt}_mocov2_meanclose_shg_lg_tuneshgy${shg_scale}_obs/ \
...
--file runs/sampling_ots/IMN64/unconditional/scale${scale}_jointtemp${jt}_margtemp${mt}_mocov2_meanclose_shg_lg_tuneshgy${shg_scale}_obs/reference/matches.npz"
echo ${cmd}
eval ${cmd}
done
done
