...
#PBS -o output_nci/logkc20.txt
#PBS -e output_nci/errorkc20.txt
...
do
  for kc in "${kcs[@]}"
  do
cmd="WORLD_SIZE=1 RANK=0 MASTER_IP=127.0.0.1 MASTER_PORT=29513 MARSV2_WHOLE_LIFE_STATE=0 python3 script_odiff/mocov2_meanclose_contrastive_sup_instance_sample_transform_nci.py \
...
--classifier_type mocov2 --model_path ${storage_dir}/models/64x64_diffusion.pt $SAMPLE_FLAGS --joint_temperature ${jt} \
 --logdir ${storage_dir}/runs/sampling_ots_rerunicml/IMN64/conditional/scale${scale}_jointtemp${jt}_margtemp${mt}_k${kc}_mocov2_meanclose_sup_contrastive_isb/ \
...
cmd="python3 evaluations/evaluator_tolog.py ${storage_dir}/reference/VIRTUAL_imagenet64_labeled.npz \
 ${storage_dir}/runs/sampling_ots_rerunicml/IMN64/conditional/scale${scale}_jointtemp${jt}_margtemp${mt}_k${kc}_mocov2_meanclose_sup_contrastive_isb/reference/samples_50000x64x64x3.npz"
echo ${cmd}
eval ${cmd}
done
done
