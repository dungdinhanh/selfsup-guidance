  ...
  do
    ...
    do
      ...
      do
        for seed in "${seeds[@]}"
        do
cmd="python script_odiff/analysis/consistency_analyse_andreal.py $MODEL_FLAGS --classifier_scale ${scale}  \
 --classifier_type mocov2 --model_path models/256x256_diffusion.pt $SAMPLE_FLAGS --joint_temperature ${jt}\
 --logdir runs/analysis/consitency2/c${fixclass}_k${kchosen}_s${seed}/scale${scale}/ \
 --features eval_models/imn256_mocov2/reps3.npz --k_closest ${kc} \
 --fix_class_index ${fixclass} --k_chosen ${kchosen} --seed ${seed}"
echo ${cmd}
eval ${cmd}
done
done
