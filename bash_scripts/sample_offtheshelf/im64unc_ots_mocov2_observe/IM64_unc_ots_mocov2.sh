...
scales=( "10.0" "12.0" "14.0" "16.0" "18.0" "20.0")
...
do
cmd="python script_odiff/mocov2_meanclose_sup_sample_transform_observe.py $MODEL_FLAGS --classifier_scale ${scale}  \
--classifier_type mocov2 --model_path models/64x64_diffusion_unc.pt $SAMPLE_FLAGS --joint_temperature ${jt} \
 --logdir runs/sampling_ots/IMN64/unconditional/scale${scale}_jointtemp${jt}_margtemp${mt}_mocov2_meanclose_obs/ \
...
--file runs/sampling_ots/IMN64/unconditional/scale${scale}_jointtemp${jt}_margtemp${mt}_mocov2_meanclose_obs/reference/matches.npz"
echo ${cmd}
eval ${cmd}
done
done
