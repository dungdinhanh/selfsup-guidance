...
do
cmd="python script_odiff/mocov2_meanrandom_contrastive_sup_instance_sample_transform.py $MODEL_FLAGS --classifier_scale ${scale}  \
--classifier_type mocov2 --model_path models/64x64_diffusion.pt $SAMPLE_FLAGS --joint_temperature ${jt} \
 --logdir runs/sampling_ots_randommean/IMN64/conditional/scale${scale}_jointtemp${jt}_margtemp${mt}_mocov2_meanclose_sup_contrastive_isb/ \
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet64_labeled.npz \
 runs/sampling_ots_randommean/IMN64/conditional/scale${scale}_jointtemp${jt}_margtemp${mt}_mocov2_meanclose_sup_contrastive_isb/reference/samples_50000x64x64x3.npz"
echo ${cmd}
eval ${cmd}
done
done
